...
**`.env.prod`** holds only the production Supabase project's `DATABASE_URL`/`DATABASE_URL_MIGRATIONS`, gitignored, never touched by `npm run dev`/`build`/`start` (those still read `.env.local`, pointed at the dev project). Copy `.env.prod.example` to `.env.prod` and fill in the prod project's connection strings once; from then on `npm run db:migrate:prod` applies pending migrations straight to prod with no swapping back and forth. If you don't already have `.env.prod` set up, get the values from the production Supabase project → **Connect** → Transaction/Session pooler, same as `.env.local`'s setup.
...
12. Add a row to the table below (date, semver, versionCode, versionName, track, notes) **now, as part of this same release — not a later follow-up.** (Past releases drifted into adding this a cycle late as catch-up; that was a process slip, not the intended flow — don't repeat it.) Commit — if `main` already deployed without this row (e.g. because the PR in step 5 was already merged), add it on `develop` and open a small docs-only PR straight into `main` immediately, same as any other post-release fix.
...
| 2026-09-11 | 1.10.0 *(web only — no Android repackage needed, nothing in this release touches the native package)* | — | — | — | Reset password by email from the login page (`/forgot-password`, `/reset-password`), single-use sha256-hashed tokens with a 1-hour expiry, sent via Resend in the recipient's own locale. Dev and production split into separate Supabase projects — `develop`/PR previews now hit a dedicated dev database, only `main` touches the real one; migrations now applied to prod via a new `.env.prod`/`npm run db:migrate:prod` as an explicit release step. New public `/changelog` page listing user-facing changes per release, linked from a new landing-page section below the feedback card. Refreshed `public/screenshots/*` and `public/landing/*`, stale since 2026-09-05. Tagged `v1.10.0` on `main`. |
